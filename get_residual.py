...
def is_tensor_data_artifact(file_name: str) -> bool:
	base_name = os.path.basename(file_name)
	if base_name.endswith(TENSOR_DATA_EXTENSIONS):
		return True
	return any(marker in base_name for marker in TENSOR_INDEX_MARKERS)


def list_local_non_tensor_artifacts(model_source: str) -> list[str]:
	artifacts = []
	for root, _, file_names in os.walk(model_source):
		for file_name in file_names:
			absolute_path = os.path.join(root, file_name)
			relative_path = os.path.relpath(absolute_path, model_source).replace(os.sep, "/")
			if not is_tensor_data_artifact(relative_path):
				artifacts.append(relative_path)
	return sorted(artifacts)


def list_remote_non_tensor_artifacts(model_source: str) -> list[str]:
	from huggingface_hub import list_repo_files

	return sorted(file_name for file_name in list_repo_files(model_source) if not is_tensor_data_artifact(file_name))


def copy_local_artifacts(model_source: str, output_dir: str, file_names: list[str]) -> None:
	for file_name in file_names:
		target_path = os.path.join(output_dir, file_name)
		os.makedirs(os.path.dirname(target_path), exist_ok=True)
		shutil.copy2(os.path.join(model_source, file_name), target_path)


def copy_remote_artifacts(model_source: str, output_dir: str, file_names: list[str]) -> None:
...
def try_copy_non_tensor_artifacts(model_source: str, output_dir: str) -> None:
	try:
		os.makedirs(output_dir, exist_ok=True)
		if os.path.isdir(model_source):
			file_names = list_local_non_tensor_artifacts(model_source)
			copy_local_artifacts(model_source, output_dir, file_names)
		else:
			file_names = list_remote_non_tensor_artifacts(model_source)
			copy_remote_artifacts(model_source, output_dir, file_names)
		if not file_names:
			print(f"Skip artifact copy: no non-tensor artifacts found in {model_source}")
	except Exception as exc:  # pylint: disable=broad-except
		print(f"Skip artifact copy: {exc}")
...
def run(args):
	...
	try_copy_non_tensor_artifacts(original_source, output_dir)
