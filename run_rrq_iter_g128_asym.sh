...
python -m auto_round --model "$MODEL_PATH" --scheme "$SCHEME"  --format "fake" --iters 0 --output_dir "$OUTPUT_DIR" "${LOW_GPU_MEM_USAGE_ARGS[@]}" --group 128 --asym
...
for ((iteration = 0; iteration < ITERATION_COUNT; iteration++)); do
	...
	python -m auto_round --model "$OUTPUT_RESIDUAL_DIR" --scheme "$SCHEME" --format "fake" --iters 0 --output_dir "$OUTPUT_RESIDUAL_QUANT_DIR" "${LOW_GPU_MEM_USAGE_ARGS[@]}" --group 128 --asym
